...
class LiquibaseChangeSetWriterUnitTests {
	...
	void fieldForFkShouldNotBeCreatedTwice() {
		...
		Optional<Change> tableWithFk = changeSet.getChanges().stream().filter(change -> change instanceof CreateTableChange
					&& "table_with_fk_field".equals(((CreateTableChange) change).getTableName())).findFirst();
	...
	void assertCreateTable(ChangeSet changeSet, String tableName, Tuple... columnTuples) {
		Optional<Change> createTableOptional = changeSet.getChanges().stream().filter(change -> change instanceof CreateTableChange && ((CreateTableChange) change).getTableName().equals(tableName)).findFirst();
	...
	void assertAddForeignKey(ChangeSet changeSet, String baseTableName, String baseColumnNames,
		...
		Optional<Change> addFkOptional = changeSet.getChanges().stream().filter(change -> change instanceof AddForeignKeyConstraintChange
					&& ((AddForeignKeyConstraintChange) change).getBaseTableName().equals(baseTableName)
					&& ((AddForeignKeyConstraintChange) change).getBaseColumnNames().equals(baseColumnNames)).findFirst();
